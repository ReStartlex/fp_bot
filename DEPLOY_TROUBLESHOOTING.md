...
5. **Проверить из Telegram:** `/start` в shop-боте → приветствие + reply-меню
   (🛍 Каталог / 🔍 Поиск / 💰 Баланс / 📦 Заказы / 👥 Рефералы / 🆘 Поддержка),
   регистрация в `shop_users`, тап «💰 Баланс» → 0₽.

6. **Включить inline-mode у бота** (для нативного поиска `@MyBot apple` в любом чате):

   ```
   В @BotFather:
   /mybots → выбрать своего shop-бота → Bot Settings → Inline Mode → Turn on
   (опционально) Inline Placeholder → "Поиск товара…"
   ```

   После этого где угодно в Telegram наберите `@<username> apple` —
   увидите подсказки прямо во всплывайке. Если выпадайка не появилась —
   подожди ≤1 мин, Telegram кэширует профиль бота.

**Откат:** выставить `SHOP_ENABLED=false` в `.env` и рестартануть — bridge-бот
продолжит работать, shop отключится.

**Sprint 2.2 — Pro UX (2026-05-25):** обновление с persistent reply-меню,
FSM-поиском, inline-mode и пагинацией категорий. После `update.sh` миграция
БД отрабатывает автоматически (см. лог: `init_db: backfill base_name/group_slug
для N услуг`). При первом старте после обновления юзеру может прилететь
старая инлайн-клавиатура — это нормально, новые сообщения уже с reply-menu.

**Sprint 2.3 — UX-pass + brand «NeuroDrop» (2026-05-25):**
- Hotfix: кнопка «« Назад к группе» теперь корректно ведёт в каталог
  для singleton-групп (где в группе всего 1 категория — drill-down
  возвращал тот же экран, Telegram игнорил `not modified`).
- Балансу — статистика (заработано/потрачено/операций) + кнопки
  пополнения (CryptoBot / Stars / Карта — пока stubs, заработают в Sprint 3)
  + история операций с пагинацией.
- Рефералам — статистика (приглашено / активных за 30д / заработано),
  «📤 Поделиться» и «📋 Скопировать ссылку» (`switch_inline_query`),
  объяснение бонусной программы.
- Брендинг: NeuroDrop, упоминание сайта `neurodrop.ru` в `/start`,
  `/help`, `/support` и inline-результатах.
