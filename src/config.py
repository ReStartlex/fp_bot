...
class Settings(BaseSettings):
    ...
    # Джиттер TTL diff-cache (P0-1 фаза A). Проблема: после общего цикла
    # у всех лотов last_synced_at ≈ одно время → TTL истекает у всех разом
    # → залп из N GET к FunPay → 429-шторм → цикл не успевает за интервалом
    # («maximum number of running instances»). Детерминированный per-lot
