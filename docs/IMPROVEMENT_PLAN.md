...
### [x] P1-1. Разбить `orders/processor.py` (1551→460, −70%) — ВСЕ ИНКР. DONE
...
- [x] Инкр. 4 (`da4cfbf`): `stages/purchase.py` — `run_purchase` (NS
  ...
  ОБА остались в processor; между ними create→pay→wait его не трогает.
  `run_purchase` возвращает dict (терминал: failed/dry_run/manual_hold)
  ЛИБО `(pins, ns_custom_id, ns_price_usd)` на успехе → терминальные
  `return {...}` скопированы дословно (минимум трансформаций). processor
  держит own_ns lifecycle + try/finally + pins_ready-save + post-pins
  гейты + доставку. Патч `purchase.session_factory` в 5 фикстурах;
  `_build_ns_fields`/`_is_valid_uuid4` ре-экспортятся (тесты их импортят).
  1153 зелёных.

**Приёмка.** Монолит 1551→460 (−70%). Все стадии (resolve/holds/delivery/
purchase) вынесены в `stages/`, processor — оркестратор (pre-checks +
hand-off). Поведение = 0 (1153 теста зелёные; боевые заказы QHLJ55KL/
XUP98WXD/TQ7C6DYA после деплоя инкр.1–3 прошли чисто). Порог «<400» не
догнан буквально (460), но цель — декомпозиция монолита — достигнута;
оставшиеся 460 строк это и есть оркестратор, дробить дальше = искусственно.

**Сложность:** средняя (инкр. 1–3 — механические; инкр. 4 — экстракция
по чистому шву).
...
### P&L по заказу: сохранённая прибыль + breakdown (2026-06-13, `9c86256`)
